...
export const getUsersByEmail = async(db:any, userEmail:string, schema:string) =>{ // eslint-disable-line @typescript-eslint/no-explicit-any
    ...
    SELECT
        ...
        tyc as tyc,
        accesos as accesos
    FROM [${process.env.NEXT_PUBLIC_S360_DB_NAME}].${schema}.${tables.s360_access} WHERE correo = @email`
    
    /*const query = `SELECT id as id, user_email as email, user_name as [user], user_company as company, fk_company as fk_company, fk_role_id as permission FROM [${process.env.NEXT_PUBLIC_S360_DB_NAME}].${schema}.${tables.users} WHERE user_email = @email`;*/
...
export const getAllUsers = async (db:any, schema:string) =>{ // eslint-disable-line @typescript-eslint/no-explicit-any
    ...
    SELECT
        ...
        tyc as tyc,
        accesos as accesos
    FROM [${process.env.NEXT_PUBLIC_S360_DB_NAME}].${schema}.${tables.s360_access}`
    console.log(query)
...
export const editUser = async (db:any, schema:string, data:userResponse) =>{ // eslint-disable-line @typescript-eslint/no-explicit-any
    ...
    const query = `UPDATE [${process.env.NEXT_PUBLIC_S360_DB_NAME}].${schema}.${tables.s360_access} SET nombre = @nombre, entidad = @entidad, correo = @correo, perfil = @perfil, accesos = @accesos WHERE id = @id`
...
export const createUser = async (db:any, schema:string, data:{nombre: string, entidad: string, correo: string, perfil: string,}) =>{ // eslint-disable-line @typescript-eslint/no-explicit-any
    ...
    const query = `INSERT INTO [${process.env.NEXT_PUBLIC_S360_DB_NAME}].${schema}.${tables.s360_access} (nombre, entidad, correo, perfil, tiempoInactividad, tyc, accesos) VALUES (@nombre, @entidad, @correo, @perfil, 15, 0, '{"Consultar ciudadano":1,"Datos actuales del ciudadano":1,"Datos históricos del ciudadano":1,"Grupo familiar del ciudadano":1,"Ver tableros":1,"Ver anomalías":1,"Consultar listados":1,"Componente Geográfico":1,"Calidad de datos":1,"Administrar usuarios":1}')`
...
export const deleteUser = async (db:any, schema:string, id:number) =>{ // eslint-disable-line @typescript-eslint/no-explicit-any
    const query = `DELETE FROM [${process.env.NEXT_PUBLIC_S360_DB_NAME}].${schema}.${tables.s360_access} WHERE id = @id`
